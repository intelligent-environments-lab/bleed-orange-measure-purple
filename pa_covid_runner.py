...
def plot_purpleair_pm(fig, files, param='PM2.5_ATM_ug/m3'):
    for file in files:
        if file.hourly[param] is not None:
            file.frequency = None
            filtered = remove_outlier(file[:], param).resample('H').mean()
...
def label_plot(fig):
    fig.update_layout(
        ...
        title='UT PurpleAir Hourly Averages',
...
def make_raw_plot(pa_files, tceq):
    ...
    # TCEQ PM2.5 
    tceq = tceq.resample(frequency)
    fig.add_trace(go.Scattergl(x=tceq.time, y=tceq.data['PM2.5'],
    ...
    label_plot(fig)
    plot(fig, filename='temp-timeseries-hourly.html')
