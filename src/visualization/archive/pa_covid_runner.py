...
def plot_purpleair_pm(fig, files, param='PM2.5_ATM_ug/m3'):
    for file in files:
        if file.hourly[param] is not None:
            ...
            time = values.index
            fig.add_trace(
                go.Scattergl(x=time, y=values, mode='lines', name=file.sensorname),
                row=1,
                col=1,
            )
...
def plot_avg_param(fig, param='Temperature_F', second_y=False, r=2, c=1):
    ...
    combined_data = [
        file.hourly[param].rename(file.sensorname)
    ...
    fig.add_trace(
        go.Scattergl(x=time, y=values, mode='lines', name=param),
        row=r,
        col=c,
...
def plot_avg_pm(fig, param='PM2.5_ATM_ug/m3', second_y=False, r=1, c=1):
    ...
    # Rolling
    combined_data = [
        file.data.resample('H')
        .mean()
        .rolling(window=100, min_periods=1, center=True)
        .mean()[param]
        .rename(file.sensorname)
        for file in pa_files
    ]
    ...
    time = values.index.to_pydatetime()
    fig.add_trace(
        go.Scattergl(x=time, y=values, mode='lines', name='PurpleAir PM2.5'),
        row=r,
        col=c,
        secondary_y=second_y,
    )
...
def plot_tceq_trh(fig, freq=None):
    ...
    fig.add_trace(
        go.Scattergl(
            x=tceq_trh.index,
            y=tceq_trh['Temperature(F)'],
            mode='lines',
            name='TCEQ Temperature',
        ),
        row=2,
        col=1,
        secondary_y=False,
    ...
    fig.add_trace(
        go.Scattergl(
            x=tceq_trh.index,
            y=tceq_trh['Relative Humidity(%)'],
            mode='lines',
            name='TCEQ Relative Humidity',
        ),
        row=2,
        col=1,
        secondary_y=True,
...
def make_raw_plot(pa_files, tceq):
    ...
    fig.add_trace(
        go.Scattergl(x=tceq_a.time, y=tceq_a['PM2.5'], mode='lines', name='TCEQ PM2.5'),
        row=1,
        col=1,
    )
