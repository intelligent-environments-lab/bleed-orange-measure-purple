...
class TCEQfile(CommonFile):
    ...
    def findPM2_5(self, rawdata):
        ...
        location = first_column.index[
            first_column == search_value
        ]  # Finds the desired parameter data
        if location.size > 0:  # Verifies that it actually found the data
            location = location[0]  # array to single value
            if (
                first_column[location + 3] == 'Date'
            ):  # Verifies that data structure is as anticipated
                target_data = rawdata.iloc[location + 3 :, :]
                target_data.reset_index(
                    drop=True, inplace=True
                )  # optional, zeros the index column
                return target_data
            else:
                return None
        else:
