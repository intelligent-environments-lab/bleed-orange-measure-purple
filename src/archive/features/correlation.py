...
tceq_trh['Time'] = pd.to_datetime(
    tceq_trh['Time'], format='%Y-%m-%d %H:%M:%S'
).dt.tz_localize('US/Central', nonexistent='NaT')
tceq_trh.set_index('Time', inplace=True)
data = plot_avg_pm()
data = pd.concat(
    [data, tceq.hourly.pm25, tceq_trh['Relative Humidity(%)']], axis=1
).dropna(how='any')
