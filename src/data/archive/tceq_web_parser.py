...
def get_site(browser):
    browser.get('https://www.tceq.texas.gov/cgi-bin/compliance/monops/yearly_summary.pl')
...
def format_time(df):
    df['Time'] = pd.to_datetime(df.iloc[:,0], format='%m/%d/%Y %H:%M')\
                    .dt.tz_localize('US/Central', ambiguous=np.repeat(False,df.shape[0]), nonexistent='shift_forward')\
                    .dt.tz_convert('UTC')\
